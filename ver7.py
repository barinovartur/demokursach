...
class ImageOverlayBot:
    ...
    def create_overlay_image(self, background: Image, overlay: Image) -> Image:
        # Конвертируем оба изображения в RGBA для работы с прозрачностью
        background = background.convert("RGBA")
        overlay = overlay.convert("RGBA")

        # Масштабируем изображение шаблона к размеру фона с использованием LANCZOS
        if overlay.size != background.size:
            overlay = overlay.resize(background.size, Image.Resampling.LANCZOS)  # Используем LANCZOS для качественного ресемплинга

        # Накладываем шаблон на фон с сохранением прозрачности
        background_resized = background.copy()
        background_resized.paste(overlay, (0, 0), overlay)  # Используем альфа-канал для прозрачности

        # Возвращаем результат без изменений формата
        return background_resized

    async def send_results(self, query):
        # Создаем временную папку для хранения изображений
        with tempfile.TemporaryDirectory() as temp_dir:
            # Сохраняем изображения как файлы PNG (без потерь качества)
            for idx, result_image in enumerate(self.result_images):
                file_path = os.path.join(temp_dir, f"result_{idx + 1}.png")
                result_image.save(file_path, format='PNG', quality=100)  # Сохраняем в PNG для лучшего качества
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text("Генерация завершена. Вы можете вернуться в главное меню.", reply_markup=reply_markup)

# Функция для запуска бота
def main():
    token = "7090094864:AAHsT-P_px-y27eqTlFlKxQ4-OcKWg94_cI"  # Вставьте свой токен бота
