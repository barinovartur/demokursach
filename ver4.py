...
class ImageOverlayBot:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                await update.message.reply_text(
                    "Ошибка: это не ZIP файл или файл поврежден. Пожалуйста, отправьте корректный архив.")
                return

            # Проверяем тип архива
            if 'expected_type' not in context.user_data:
                await update.message.reply_text(
                    "Ошибка: не указан тип архива. Пожалуйста, отправьте архив с фонами или шаблонами.")
                return
    ...
    async def process_overlay_zip(self, temp_dir: str, update: Update):
        ...
        if found_overlay:
            ...
            await update.message.reply_text(
                "Не было найдено шаблонов в архиве. Пожалуйста, отправьте архив с шаблонами.")
    ...
    async def send_results(self, query):
        for idx, result_image in enumerate(self.result_images):
            with BytesIO() as output:
                ...
                output.seek(0)
                await query.message.reply_photo(photo=InputFile(output), caption=f"Генерация {idx + 1}")
...
if __name__ == '__main__':
    bot = ImageOverlayBot('7090094864:AAHsT-P_px-y27eqTlFlKxQ4-OcKWg94_cI')  # Замените на ваш токен
    bot.run()
