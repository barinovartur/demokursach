...
class ImageOverlayBot:
    ...
    async def handle_photo(self, update: Update, context: CallbackContext):
        ...
        if self.expected_type == 'background':
            ...
            await update.message.reply_text("Фон получен.")
        else:
            await update.message.reply_text("Ошибка: фото принимаются только для фонов.")
    ...
    async def handle_png_document(self, update: Update, context: CallbackContext):
        ...
        await update.message.reply_text("Шаблон (PNG) получен.")
    ...
    async def process_background_zip(self, temp_dir: str, update: Update):
        ...
        if found_backgrounds:
            ...
            await update.message.reply_text("Не было найдено фонов в архиве.")
    ...
    async def process_overlay_zip(self, temp_dir: str, update: Update):
        ...
        if found_overlay:
            await update.message.reply_text(f"Шаблоны (PNG) получены: {len(self.overlay_images)}.")
        else:
            await update.message.reply_text("В архиве не найдено PNG изображений.")
